...
``` {r me-treatement-event34, echo = FALSE}
...
m3ME <- lmerTest::lmer(formula = estDryMass ~ treatment + (1|clipID),
                       data = longDF,
                       REML = FALSE)
...
m1LM <- lm(estDryMass ~ exclosure*treatment, data = longDF)
summary(m1LM)
anova(m1LM)
...
1. Summary output for Mixed Effects model indicates that D11 Clip Strips can be subsampled at the 15% level for all bouts with **herbGroup** = ALL.
