...
pointChecker.func <- function(input, ply = llply){
  ply(input, function(x){
    ...
    # match and append azimuth values range from lookup table, these columns are temporary
...
names(missingNames.output) <- c("NA","Blank","Unknown")
missingNames.output$siteID = siteID.real
kable(missingNames.output[,c(4,1,2,3)], caption = "Missing Scientific Names by Site") # re-arrange columns
```
