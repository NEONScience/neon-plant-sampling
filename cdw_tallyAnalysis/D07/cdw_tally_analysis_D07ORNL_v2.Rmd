---
title: "CDW Tally Analysis: D07 ORNL"
...
if (file.exists("~/Documents/neonScienceDocs")){
  outpath <- paste("~/Documents/neonScienceDocs/gitRepositories/neonPlantSampling/", domain, sep="")
}

if (file.exists("/Users/ssimkin")){
  outpath <- "C:/GitHub/neon-plant-sampling/"
  plot_type <- read.table(paste0(outpath,"spatialData/plotSpatialData_20180827.csv"), sep=",", header=TRUE, stringsAsFactors = F)
  outpath <- paste0(outpath,"cdw_tallyAnalysis/", domain)
}
...
cdwQuery = paste(URLencode('SELECT * FROM "CDW: Tally [PROD]" AS parent
                      JOIN "CDW: Tally [PROD]/cdw_transect" AS child'),
...
cdw %>% 
  ...
  dplyr::filter(siteid==site) %>% dplyr::filter(taxonid!="JUVI") -> cdw

 # filtered out taxonID JUVI since Field Science said this was always something else, typically a Pinus species
...
plot_type <- subset(plot_type, subtype=="basePlot") # filter to just basePlots (e.g. remove tick, bird, mammal, etc. plots)
  plot_type <- subset(plot_type, select=c(plotID, plotType)) # show just the plotID and corresponding plotType (tower vs. distributed)
cdw <- merge(cdw, plot_type, by.x="plotid_parent", by.y="plotID", all.x=TRUE) # add plotType to cdw dataframe
#  cdw <- subset(cdw, select=c(plotid_parent, tallydate, plotType)) # temporarilly subset to view the various date by towerType combinations
cdw <- subset(cdw, plotType=="distributed" | (plotType=="tower" & substr(tallydate, 1,4)=="2019")) 
    # filter out 2015 and 2017 tower data that has been replaced by taxonomically improved 2019 tower data
cdw <- subset(cdw, !is.na(diameterClass)) # some records are missing maxlogdiameter and therefore diameterClass, and without diameterClass records shouldn't be in DST summary
  cdw$plotType <- NULL # after subsetting the plotType variable is no longer necessary
