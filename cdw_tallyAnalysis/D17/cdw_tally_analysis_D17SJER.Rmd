...
# Define CDW Fulcrum query for D17
cdwQuery = paste(URLencode('SELECT * FROM "(TOS) Coarse Downed Wood: Tally [PROD]" AS parent
            ...
            URLencode("ON (parent._record_id = child._record_id)
                      WHERE domainid LIKE 'D17'"), sep = "%20")
...
##  Select desired fields from 'cdw' data frame, then select data for SJER only
cdw %>% 
  dplyr::select(domainid, siteid, plotid_parent, tallydate, volumefactor, particle_count, lidsazimuth, logdiameter,
                     taxonid, decayclass, logid_ingest, logdistance, loglength, acceptedtaxonid, targettaxapresent) %>%
  dplyr::filter(siteid=="SJER") -> cdw
