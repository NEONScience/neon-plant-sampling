---
title: "CDW Tally Analysis: D06 UKFS"
...
# Write-out to csv to perform taxonID updates
write.csv(cdw, file = "D06_UKFS_inputTally.csv", row.names = FALSE, fileEncoding = "UTF-8")

# Read-in csv with updated taxonIDs
cdw <- read.csv("D06_UKFS_outputTally.csv", header=TRUE, stringsAsFactors = FALSE)

```


```{r data_processing, results='hold'}
##  Filter to analyze data from 2018 only
cdw %>% filter(grepl("2018", eventid)) -> cdw
