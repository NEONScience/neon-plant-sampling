---
title: "CDW Tally Analysis: D08 DELA"
...
loglength, taxonid, scientificname, decayclass FROM "CDW: Tally [PROD]" AS parent
...
# Get CDW site data from Fulcrum
...
##  Filter and bind to analyze data from 2016 for Dist plots, and 2019 for Tower plots
amPath <- "~/Documents/gitRepositories/devTOS/spatialData/supportingDocs/"
am <- read.csv(paste0(amPath, "applicableModules.csv"), header = TRUE, stringsAsFactors = FALSE)
am %>% filter(grepl("cdw", applicableModules), siteID == "DELA") -> am
cdw <- left_join(cdw, am, by = c("plotid_parent" = "plotID")) %>% arrange (eventid, plotid_parent)
cdw %>% filter(plotType == "distributed") -> input2016
cdw %>% filter(eventid == "CDW.2019.DELA") -> input2019
inputData <- bind_rows(input2016, input2019)

#   inputData look good, convert to 'cdw' dataframe name so code below works
cdw <- inputData

write.csv(cdw, file=paste(outpath, "DELA_inputData_20190812.csv", sep = "/"), row.names = FALSE)
...
#cdw[cdw$taxonid=='2PLANT', 'taxonid'] <- '2PLANT-H'
#cdw %>% mutate(taxonid = replace(taxonid, taxonid=='2PLANT', '2PLANT-H')) -> cdw
...
cdw_fil <- dplyr::filter(cdw, targettaxapresent == "Y", !is.na(taxonid), !is.na(decayclass), decayclass!="NA")
