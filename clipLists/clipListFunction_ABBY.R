...
# Version 3 of this function creates an .xlsx file as output, one file per site, rather than per plot .csv files. This change enables technicians to edit Clip Lists directly on Sharepoint. Randomized Clip Lists from all Distributed Plots are written to one tab in the xlsx file, and lists from Tower Plots are in another tab. For Herbaceous Biogeochemistry, a third tab is created that lists all plots sampled for Canopy Foliar Chemistry sampling, and if some of these plots are Distributed Plots that would not otherwise support HBP, a fourth tab is created for Clip Lists for these plots. This is necessary because some forested plots can have only 25% forest cover, and clipping would be necessary for CFC.

# Version 4 of this function utilizes the 'openxlsx' package to create .xlsx files; previously, the 'xlsx' package was used and Java dependencies became unresolvable after an update to R. The setwd() functions were also removed, and file paths utilized instead to more explicitly state sources of inputs and destinations for outputs from function.

# Version ABBY: Makes Clip Lists for only those Distributed Plots classified according to NLCD = Forest. NLCD designations at ABBY are frequently incorrect due to logging and regrowth, so Clip Lists are needed for all plots. Output from this script is meant to be appended to existing Clip List on Sharepoint.
...
clipList <- function(siteCode){
  ...
  # Read in exclusion areas that do not support HBP sampling
  nestedCoords <- read.csv(nc, header=T, stringsAsFactors=F)
  ...
  # Filter `acceptedPlot` to obtain Distributed basePlots with applicableModules==`cfc` and nlcdClass != 'forest'
  ...
  cfcHbpPlots <- plyr::rbind.fill(cfcHbpDistPlots, cfcTowerPlots)
  cfcHbpPlots %>% select(plotID, nlcdClass, plotType, applicableModules, plotSize) -> cfcHbpPlots
  
  
  ##  If/else statement to deal with situation when all `cfc` plots already have regular lists; that is, site has no forested `cfc` Distributed Plots
  if(nrow(cfcForestDist)==0){
    cfcMessage <- paste("All Canopy Foliar Chemistry (CFC) sampling at", siteCode, 
                        "occurs in plots in which HBP is also sampled. No additional CFC Clip Lists required.", sep = " ")
    print(cfcMessage)
    } else {
      # Extract plotIDs from cfcForestDist, and sort
      cfcDistID <- sort(as.character(cfcForestDist$plotID))
      
      # Create data frame to hold CFC list output
      cfc <- data.frame()
      
      # `for` loop to generate random clip-harvest locations for each plot listed in `cfcDistID`
      for (i in 1:length(cfcDistID)){
        # Create an 8-column dataframe to hold clipID, subplotID, and the easting and northing offsets for the SW corner of the clip-strip;
        # additional 'cell' fields are employed to filter out cells that overlap exclusion zones within the plot, and these fields are 
        # deleted with code below before the .csv is written.
        ...
        ##  Create x-axis and y-axis coordinates for the SW corner of each clip-harvest strip within each grid cell and add to the dataframe.
        ##  The clip strip itself is 0.1m W x 2m H oriented North/South, and is centered within the 0.5m x 3m grid cell such that there is a 0.2m 
        ##  buffer on the West/East sides of the strip, and a 0.5m buffer on the North/South sides of the strip.
        ...
        # Calculate SW corner of each clip-strip within each grid cell, as well as grid cell corner positions, and add to the data frame created above.
        # Grid cell corner positions are needed to determine whether individual cells overlap %cover nested subplots.
        ...
        ##  Remove rows of "grid.df" that overlap 10m2 nested subplots used for Veg Structure measurements, and 9m2 area that includes 1m2 nested subs
        ##  used for %cover estimates in the plant diversity protocol; ignore the 16cm of the 10m2 subplot that overlaps the 50 cm grid cell buffer, 
        ##  as technicians will position themselves on the West/East side of clip-strips for harvesting.
        ...
        # Using location of nested subplots specified in "nestedTemp" table, identify overlapping grid cells with a 'for' loop, store associated 
        # gridIDs in "gridOmit"; then remove elements of "gridOmit" from "grid.df" and store in a new dataframe named "clip.df"
        ...
        # Append `clip.df` to `cfc` data frame
        cfc <- plyr::rbind.fill(cfc, clip.df)
        
      # End distributed plot forested `cfc` 'for' loop  
      }
      
      # Create plotID column in `cfc` to enable filtering by plotID in .xlsx file
      cfc$plotID <- str_sub(cfc$clipID, start=1, end=8)
      
      print("CFC forested plot Clip List output complete.")
      
    # End no forested plots `if` statement
    }
  ...
  # Create data frame to hold Distributed Plot list output
  distHbp <- data.frame()
  ...
  if (length(distID)==0){
    ...
    for (i in 1:length(distID)){
      ...
      # Create an 8-column dataframe to hold clipID, subplotID, and the easting and northing offsets for the SW corner of the clip-strip; 
      # additional 'cell' fields are employed to filter out cells that overlap exclusion zones within the plot, and these fields are deleted
      # with code below before the .csv is written.
      grid.df <- data.frame(matrix(data=NA, nrow=totalCell, ncol=8), stringsAsFactors=FALSE)
      cNames <- c("clipID","subplotID","offsetEasting","offsetNorthing","x1Cell","x2Cell","y1Cell","y2Cell")
      colnames(grid.df) <- cNames
      grid.df$clipID <- as.character(grid.df$clipID)
      ...
      # Create x-axis and y-axis coordinates for the SW corner of each clip-harvest strip within each grid cell and add to the dataframe. 
      # The clip strip itself is 0.1m W x 2m H oriented North/South, and is centered within the 0.5m x 3m grid cell such that there is a 0.2m
      # buffer on the West/East sides of the strip, and a 0.5m buffer on the North/South sides of the strip. Define X and Y coords of SW corner
      # of grid cells in each row of cells and each column of cells within the plot
      xTemp <- seq(from=bufferWidth, to=xDistance+cellWidth, by=cellWidth)
      yTemp <- seq(from=bufferWidth, to=(yDistance-1), by=cellHeight)
      ...
      ##  Remove rows of "grid.df" that overlap 10m2 nested subplots used for Veg Structure measurements, and 9m2 area that includes 1m2 nested
      ##  subs used for %cover estimates in the plant diversity protocol; ignore the 16cm of the 10m2 subplot that overlaps the 50 cm grid cell
      ##  buffer, as technicians will position themselves on the West/East side of clip-strips for harvesting.
      # Obtain coordinates for nested subplots associated with 20m x 20m distributed plots
      nestedTemp <- filter(nestedCoords, subplotID==31)
      ...
      # Append `clip.df` to `distHbp` data frame
      distHbp <- plyr::rbind.fill(distHbp, clip.df)      
      
    # End distributed plot 'for' loop  
    }
  
    # Create plotID column in `distHbp` to enable filtering by plotID in .xlsx file
    distHbp$plotID <- str_sub(distHbp$clipID, start=1, end=8)
  ...
  # Create data frame to hold Tower Plot list output
  towerHbp <- data.frame()
  ...
  # plotID. Then create a dataframe to associate values of gridStart with subplotIDs. This dataframe is used below to pull the correct value of 
  # gridStart for randomly sampled values of subplotID.
  gridStart <- c()
  gridInit <- 0
  ...
  for (i in 1:length(towerID)){
    ...
    for (j in 1:length(theChosen)){
      ...
      #	Create gridIDs specific to the subplotID, matching values of subplotID against those in gridStart.df to select a starting point for the
      # gridID sequence that ensures gridIDs do not overlap for any subplotID within a given plotID. Then create clipIDs from the gridIDs, and
      # add clipIDs and subplotIDs to the grid.df dataframe.
      ...
      # Create x-axis and y-axis coordinates for the SW corner of each clip-harvest strip within each grid cell and add to the dataframe. 
      # The clip strip itself is 0.1m W x 2m H oriented North/South, and is centered within the 0.5m x 3m grid cell such that there is a 0.2m buffer
      # on the West/East sides of the strip, and a 0.5m buffer on the North/South sides of the strip. Define X and Y coords of SW corner of 
      # grid cells in each row of cells and each column of cells within the plot.
      xTemp <- seq(from=bufferWidth, to=xDistance+cellWidth, by=cellWidth)
      yTemp <- seq(from=bufferWidth, to=(yDistance-1), by=cellHeight)
      ...
      # Using location of nested subplots specified in "nestedCoords" table for the selected subplotID within the larger tower baseplot, identify 
      # overlapping grid cells with a 'for' loop, store associated gridIDs in "gridOmit"; then remove elements of "gridOmit" from "grid.df" and store
      # in a new dataframe named "clip.df".
      ...
      # Append `clip.df` to `distHbp` data frame
      towerHbp <- plyr::rbind.fill(towerHbp, clip.df)
  ...
  # Create plotID column in `towerHbp` to enable filtering by plotID in .xlsx file
  towerHbp$plotID <- str_sub(towerHbp$clipID, start=1, end=8)
  
  print("Tower plot output complete")
  
  
  
  ### Write output to .xlsx file using 'openxlsx' package
  ##  Re-order column names in data frames for output
  # Re-order columns in 'cfc' data frame, for writing to 'cfc_forestDistributed' tab
  if(nrow(cfcForestDist)!=0){cfc <- cfc[c("plotID", "subplotID", "clipID", "offsetEasting", "offsetNorthing", "status", "date")]}
  
  # Re-order columns in 'distHbp' data frame, for writing to 'distributed' tab
  if(length(distID) > 0){distHbp <- distHbp[c("plotID", "subplotID", "clipID", "offsetEasting", "offsetNorthing", "status", "date")]}
  
  # Re-order columns in 'towerHbp' data frame, for writing to 'tower' tab
  towerHbp <- towerHbp[c("plotID", "subplotID", "clipID", "offsetEasting", "offsetNorthing", "status", "date")]
  
  # Define output file name
  outputName <- paste0(siteCode,"_clipList.xlsx")
  
  # Create list of sheetNames and associated data inputs
  if(length(distID) > 0 & nrow(cfcForestDist)!=0){
    listOutput <- list("cfc_tower_nonForestDist" = cfcHbpPlots, "cfc_forestDistributed" = cfc, "distributed" = distHbp, "tower" = towerHbp)
    rowNameInput <- c(FALSE, TRUE, TRUE, TRUE)
  } else if(length(distID) > 0 & nrow(cfcForestDist)==0){
    listOutput <- list("cfc_tower_nonForestDist" = cfcHbpPlots, "distributed" = distHbp, "tower" = towerHbp)
    rowNameInput <- c(FALSE, TRUE, TRUE)
  } else if(length(distID)==0 & nrow(cfcForestDist)!=0){
    listOutput <- list("cfc_tower_nonForestDist" = cfcHbpPlots, "cfc_forestDistributed" = cfc, "tower" = towerHbp)
    rowNameInput <- c(FALSE, TRUE, TRUE)
  } else {
    listOutput <- list("cfc_tower_nonForestDist" = cfcHbpPlots, "tower" = towerHbp)
    rowNameInput <- c(FALSE, TRUE)
  }
  
  # Write out to .xlsx file
  write.xlsx(listOutput, file = paste(writePath, outputName, sep = "/"), 
             colNames = TRUE, rowNames = rowNameInput, keepNA = FALSE, firstRow = TRUE)
