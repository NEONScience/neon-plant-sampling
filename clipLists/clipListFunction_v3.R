...
# Set output working directory, and define output file name
...
# Write list of cfcHbpPlots to .xlsx tab as a reference for techs
cfcHbpName <- "cfc_Tower_nonForestDist"
write.xlsx2(cfcHbpPlots, file=outputName, sheetName = cfcHbpName, col.names = TRUE, row.names = FALSE, append = TRUE, showNA = FALSE)
...
# Re-order columns in `cfc`, and write to `cfc_forestDistributed` tab in .xlsx file
cfc <- cfc[c("plotID", "subplotID", "clipID", "offsetEasting", "offsetNorthing", "status", "date")]
write.xlsx2(cfc, file = outputName, sheetName = "cfc_forestDistributed", col.names = TRUE, row.names = TRUE, append = TRUE, showNA = FALSE)
...
# If/else to deal with possibility that no plotIDs exist within potentially sample-able nlcdClasses at the site.
  if (length(distID)==0){
    ...
    # Re-order columns in `distHbp`, and write to `distributed` tab in .xlsx file
    distHbp <- distHbp[c("plotID", "subplotID", "clipID", "offsetEasting", "offsetNorthing", "status", "date")]
    write.xlsx2(distHbp, file = outputName, sheetName = "distributed", col.names = TRUE, row.names = TRUE, append = TRUE, showNA = FALSE)
...
# Re-order columns in `towerHbp`, and write to `tower` tab in .xlsx file
towerHbp <- towerHbp[c("plotID", "subplotID", "clipID", "offsetEasting", "offsetNorthing", "status", "date")]
write.xlsx2(towerHbp, file = outputName, sheetName = "tower", col.names = TRUE, row.names = TRUE, append = TRUE, showNA = FALSE)
