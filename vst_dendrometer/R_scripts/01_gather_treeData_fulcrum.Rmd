...
plantstatus FROM "VST: Apparent Individuals [PROD]" AS Parent'),
                    ...
                    URLencode("AND siteid IN ('BONA', 'DEJU', 'HEAL', 'NIWO', 'RMNP', 'SJER', 'SOAP', 'SRER', 'TEAK', 'YELL', 'WREF')"),
...
vstMapQuery <- paste(URLencode('SELECT eventid, domainid, siteid, plotid, subplotid, pointid, stemazimuth, stemdistance, taxonid, individualid FROM "VST: Mapping and Tagging [PROD]"'),
                     URLencode("WHERE siteid IN ('BONA', 'DEJU', 'HEAL', 'NIWO', 'RMNP', 'SJER', 'SOAP', 'SRER', 'TEAK', 'YELL', 'WREF')"),
...
#write.csv(df_out, paste(wdir, "vst_dendrometer/SourceData/vst_data_d_cleaned.csv"), row.names=FALSE)
